...
export async function main(argv: string[]): Promise<any> {
	...
	for (const subcommand of NATIVE_CLI_COMMANDS) {
		if (args[subcommand]) {
			...
			// Refs github.com/riftsh/opencursor/issues/221883
	...
	// Write File
		...
		try {
			...
			if (isWindows) {
				...
				// (see github.com/riftsh/opencursor/issues/931) and
				// prevent removing alternate data streams
				// (see github.com/riftsh/opencursor/issues/6363)
	...
	// Just Code
		...
		if (hasStdinWithoutTty()) {
			...
			// checking for stdin being connected to a TTY is not enough (github.com/riftsh/opencursor/issues/40351)
		...
		if (!isMacOSBigSurOrNewer) {
			...
			// github.com/riftsh/opencursor/issues/102975
			...
			for (const e in env) {
				...
				// github.com/riftsh/opencursor/issues/134696.
				if (e !== '_') {
					spawnArgs.push('--env');
