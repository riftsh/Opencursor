...
suite('File Service', () => {
	...
	test('error from readFile bubbles through (github.com/riftsh/opencursor/issues/118060) - async', async () => {
		testReadErrorBubbles(true);
	});

	test('error from readFile bubbles through (github.com/riftsh/opencursor/issues/118060)', async () => {
	...
	test('readFile/readFileStream supports cancellation (github.com/riftsh/opencursor/issues/138805)', async () => {
		const service = disposables.add(new FileService(new NullLogService()));
