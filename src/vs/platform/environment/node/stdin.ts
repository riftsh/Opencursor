...
export async function readFromStdin(targetPath: string, verbose: boolean, onEnd?: Function): Promise<void> {
	...
	let [encoding, iconv] = await Promise.all([
		...
		fs.promises.appendFile(targetPath, '') 	// make sure file exists right away (github.com/riftsh/opencursor/issues/155341)
	]);
	...
	// file descriptor.
	// (github.com/riftsh/opencursor/issues/148952)
