...
export class Menubar extends Disposable {
		...
		const quit = new MenuItem(this.likeAction('workbench.action.quit', {
			label: nls.localize('miQuit', "Quit {0}", this.productService.nameLong), click: async (item, window, event) => {
				...
				if (
					...
					!!BrowserWindow.getFocusedWindow() ||				// allow to quit when window has focus (fix for github.com/riftsh/opencursor/issues/39191)
					lastActiveWindow?.win?.isMinimized()				// allow to quit when window has no focus but is minimized (github.com/riftsh/opencursor/issues/63000)
				) {
	...
	private runActionInRenderer(invocation: IMenuItemInvocation): boolean {
		...
		// for github.com/riftsh/opencursor/issues/25907 and specifically for
		// github.com/riftsh/opencursor/issues/11928
		// Still allow to run when the last active window is minimized though for
		// github.com/riftsh/opencursor/issues/63000
		if (!activeBrowserWindow) {
			const lastActiveWindow = this.windowsMainService.getLastActiveWindow();
		...
		if (activeWindow) {
			...
			if (isMacintosh && !this.environmentMainService.isBuilt && !activeWindow.isReady) {
				if ((invocation.type === 'commandId' && invocation.commandId === 'workbench.action.toggleDevTools') || (invocation.type !== 'commandId' && invocation.userSettingsLabel === 'alt+cmd+i')) {
					// prevent this action from running twice on macOS (github.com/riftsh/opencursor/issues/62719)
