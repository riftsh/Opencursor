...
export class KeybindingResolver {
	...
	constructor(
		...
		for (let i = 0, len = this._keybindings.length; i < len; i++) {
			...
			// substitute with constants that are registered after startup - github.com/riftsh/opencursor/issues/174218#issuecomment-1437972127
	...
	private _getKeybindingPriority(item: ResolvedKeybindingItem): number {
		...
		if (item.extensionId === 'opencursor.opencursor') {
			// todo: remove id hardcode and extract extension id from env or settings
			return 2; // OpenCursor keybindings
		}
	...
	public static handleRemovals(rules: ResolvedKeybindingItem[]): ResolvedKeybindingItem[] {
    	...
    	return result.sort((a, b) => {
			const priorityA = a.isDefault ? (a.extensionId === 'opencursor.opencursor' ? 2 : 1) : 3;
			const priorityB = b.isDefault ? (b.extensionId === 'opencursor.opencursor' ? 2 : 1) : 3;
