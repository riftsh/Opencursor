...
import { auxiliaryBarAllowedViewContainerIDs } from './opencursorViewsShared.js';

export class OpenCursorViewDescriptorService extends ViewDescriptorService implements IViewDescriptorService {
	constructor(
  ...
  override moveViewContainerToLocation(
    ...
    const isAllowed = () => {
        return auxiliaryBarAllowedViewContainerIDs.some((id: string) => viewContainer.id.includes(id));
    };
    ...
    // Prevent OpenCursor integrations from moving out of AuxiliaryBar
    if (location !== ViewContainerLocation.AuxiliaryBar && isAllowed()) {
      return;
    }
