...
suite('EditorService', () => {
	...
	test('openEditor() - same input does not cancel previous one - github.com/riftsh/opencursor/issues/136684', async () => {
		const [, service] = await createEditorService();
	...
	test('side by side editor is not matching all other editors (github.com/riftsh/opencursor/issues/132859)', async () => {
		const [part, service] = await createEditorService();
