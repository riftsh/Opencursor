...
export class TextFileEditorModel extends BaseTextEditorModel implements ITextFileEditorModel {
	...
	private async onDidFilesChange(e: FileChangesEvent): Promise<void> {
		...
		if (fileEventImpactsModel && this.inOrphanMode !== newInOrphanModeGuess) {
			...
			if (newInOrphanModeGuess) {
				...
				// exists (network shares issue: github.com/riftsh/opencursor/issues/13665).
	...
	protected override installModelListeners(model: ITextModel): void {
		...
		// See github.com/riftsh/opencursor/issues/30189
	...
	private onModelContentChanged(model: ITextModel, isUndoingOrRedoing: boolean): void {
		...
		// github.com/riftsh/opencursor/issues/102542
		...
		return this.saveSequentializer.run(versionId, (async () => {
			...
			if (this.isResolved() && !options.skipSaveParticipants) {
				try {
					...
					//
					// Reported as: github.com/riftsh/opencursor/issues/102542
			...
			// saving contents to disk that are stale (see github.com/riftsh/opencursor/issues/50942).
	...
	private async onMaybeShouldChangeEncoding(): Promise<void> {
		...
		//
		// (see github.com/riftsh/opencursor/issues/127936)
