...
suite('LanguageModelToolsService', () => {
	...
	test('invokeTool blocked by policy', async () => {
		await configurationService.setUserConfiguration('chat.openCursor.tools.blocked', ['blockedTool']);
		store.add(service.registerToolData({
			id: 'blockedTool',
			modelDescription: 'Blocked Tool',
			displayName: 'Blocked Tool'
		}));
		store.add(service.registerToolImplementation('blockedTool', {
		...
		await assert.rejects(() => service.invokeTool(dto, async () => 0, CancellationToken.None));
	});
});
