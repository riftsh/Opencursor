...
suite('ChatService', () => {
	...
	test('provider fallback retries when enabled', async () => {
		await configurationService.setUserConfiguration('chat.openCursor.providerRouting.enabled', true);
		await configurationService.setUserConfiguration('chat.openCursor.providerRouting.maxFallbackAttempts', 2);

		let attempts = 0;
		testDisposables.add(chatAgentService.registerAgent('retryAgent', { ...getAgentData('retryAgent'), isDefault: true }));
		testDisposables.add(chatAgentService.registerAgentImplementation('retryAgent', {
			async invoke() {
				attempts += 1;
				if (attempts === 1) {
		...
		const response = await testService.sendRequest(model.sessionId, 'retry please', { agentId: 'retryAgent' });
		assert(response);
