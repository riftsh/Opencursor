...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
	...
	async invokeTool(dto: IToolInvocation, countTokens: CountTokensCallback, token: CancellationToken): Promise<IToolResult> {
		...
		const blockedTools = new Set(this._configurationService.getValue<string[]>('chat.openCursor.tools.blocked') ?? []);
		if (blockedTools.has(dto.toolId)) {
			throw new Error(localize('chat.tools.blocked', 'Tool "{0}" is blocked by OpenCursor safety policy.', dto.toolId));
		}
		...
		try {
			if (dto.context) {
				...
				const forceConfirmTools = !!this._configurationService.getValue<boolean>('chat.openCursor.tools.forceConfirmation');
				if (prepared?.confirmationMessages || forceConfirmTools) {
					// We do not need the unused confirmation object here.
				...
				if (prepared?.confirmationMessages || forceConfirmTools) {
					const confirmation = prepared?.confirmationMessages ?? {
						title: localize('chat.tools.forceConfirmation.title', 'Confirm tool execution'),
						message: localize('chat.tools.forceConfirmation.message', 'OpenCursor requires confirmation before running this tool.')
					};
					const result = await this._dialogService.confirm({ message: confirmation.title, detail: renderStringAsPlaintext(confirmation.message) });
					if (!result.confirmed) {
						throw new CancellationError();
					}
				}
