...
export class ChatService extends Disposable implements IChatService {
	...
	private readonly _sessionMemory = new Map<string, string[]>();
	...
	private _sendRequestAsync(model: ChatModel, sessionId: string, parsedRequest: IParsedChatRequest, attempt: number, enableCommandDetection: boolean, defaultAgent: IChatAgent, location: ChatAgentLocation, options?: IChatSendRequestOptions): IChatSendRequestResponseState {
		...
		const sendRequestInternal = async () => {
			const workflowEnabled = !!this.configurationService.getValue<boolean>('chat.openCursor.workflow.enabled');
			const routingEnabled = !!this.configurationService.getValue<boolean>('chat.openCursor.providerRouting.enabled');
			const routingMode = this.configurationService.getValue<string>('chat.openCursor.providerRouting.mode') ?? 'balanced';
			const aiProfile = this.configurationService.getValue<string>('chat.openCursor.aiProfile') ?? 'balanced';
			const configuredMaxFallbackAttempts = Math.max(1, this.configurationService.getValue<number>('chat.openCursor.providerRouting.maxFallbackAttempts') ?? 1);
			const maxFallbackAttempts =
				aiProfile === 'rapid' ? 1 :
					aiProfile === 'deep' ? Math.min(5, configuredMaxFallbackAttempts + 1) :
						configuredMaxFallbackAttempts;
			const memoryEnabled = !!this.configurationService.getValue<boolean>('chat.openCursor.memory.enabled');
			const maxMemoryEntries = Math.max(0, this.configurationService.getValue<number>('chat.openCursor.memory.maxEntries') ?? 8);
			const autodetectBias = this.configurationService.getValue<string>('chat.openCursor.agent.autodetectBias') ?? 'medium';
			...
			try {
				...
				if (agentPart || (defaultAgent && !commandPart)) {
					const prepareChatAgentRequest = async (agent: IChatAgentData, command?: IChatAgentCommand, enableCommandDetection?: boolean, chatRequest?: ChatRequestModel, isParticipantDetected?: boolean): Promise<IChatAgentRequest> => {
						...
						return {
							...
							metadata: {
								...
								isBackgroundRun: options?.isBackgroundRun,
								routingEnabled,
								routingMode,
								aiProfile,
								autodetectBias,
								memorySnapshot: memoryEnabled ? (this._sessionMemory.get(sessionId) ?? []) : []
							}
						} satisfies IChatAgentRequest;
					};

					const shouldDetectParticipant =
						...
						!agentPart &&
						!commandPart &&
						(enableCommandDetection || autodetectBias === 'high');
					if (shouldDetectParticipant) {
					...
					completeResponseCreated();
					const attemptCount = routingEnabled ? maxFallbackAttempts : 1;
					let agentResult: IChatAgentResult | undefined;
					for (let invokeAttempt = 1; invokeAttempt <= attemptCount; invokeAttempt++) {
						if (workflowEnabled) {
							progressCallback({ kind: 'workflowPhase', phase: invokeAttempt === 1 ? 'planning' : 'execution', attempt: invokeAttempt, message: localize('chat.workflow.invokeAttempt', 'Invoking participant (attempt {0})', invokeAttempt) });
						}

						try {
							agentResult = await this.chatAgentService.invokeAgent(agent.id, { ...requestProps, attempt: invokeAttempt }, progressCallback, history, token);
							break;
						} catch (error) {
							if (invokeAttempt >= attemptCount) {
								throw error;
							}
						}
					}
				...
				if (token.isCancellationRequested) {
					...
					if (memoryEnabled && maxMemoryEntries > 0) {
						const memory = this._sessionMemory.get(sessionId) ?? [];
						memory.push(getPromptText(request.message).message);
						if (memory.length > maxMemoryEntries) {
							memory.splice(0, memory.length - maxMemoryEntries);
						}
						this._sessionMemory.set(sessionId, memory);
					}
					if (workflowEnabled) {
						progressCallback({ kind: 'workflowPhase', phase: 'complete', attempt, message: localize('chat.workflow.complete', 'Workflow completed') });
					}
					completeResponseCreated();
				...
				if (this.configurationService.getValue<boolean>('chat.openCursor.workflow.enabled')) {
					progressCallback({ kind: 'workflowPhase', phase: 'failed', attempt, message: localize('chat.workflow.failed', 'Workflow failed') });
				}
				const result = 'error';
				this.telemetryService.publicLog2<ChatProviderInvokedEvent, ChatProviderInvokedClassification>('interactiveSessionProviderInvoked', {
					timeToFirstProgress: undefined,
