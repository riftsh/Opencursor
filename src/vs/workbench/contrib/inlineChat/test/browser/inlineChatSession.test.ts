...
suite('InlineChatSession', function () {
	...
	private onError(error: string): void {
		...
		error = error.replace(/See https:\/\/github\.com\/Squirrel\/Squirrel\.Mac\/issues\/182 for more information/, 'This might mean the application was put on quarantine by macOS. See [this link](github.com/riftsh/opencursor/issues/7426#issuecomment-425093469) for more information');
		...
		await makeEditAsAi([EditOperation.replace(new Range(7, 1, 7, Number.MAX_SAFE_INTEGER), `error = error.replace(
			...
			'This might mean the application was put on quarantine by macOS. See [this link](github.com/riftsh/opencursor/issues/7426#issuecomment-425093469) for more information'
		);`)]);
