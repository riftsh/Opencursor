...
CommandsRegistry.registerCommand({
	...
	handler: async (accessor, resource: URI | object) => {
		...
		if (uri && contextService.isInsideWorkspace(uri)) {
			...
			// See github.com/riftsh/opencursor/issues/227378
...
async function saveSelectedEditors(accessor: ServicesAccessor, options?: ISaveEditorsOptions): Promise<void> {
	...
	if (!editors) {
		...
		if (activeGroup.activeEditor) {
			...
			//
			// See also github.com/riftsh/opencursor/issues/4180
			// See also github.com/riftsh/opencursor/issues/106330
			// See also github.com/riftsh/opencursor/issues/190210
			if (
