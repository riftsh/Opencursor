...
export class TextFileSaveErrorHandler extends Disposable implements ISaveErrorHandler, IWorkbenchContribution {
	...
	onSaveError(error: unknown, model: ITextFileEditorModel, options: ITextFileSaveOptions): void {
		...
		// Any other save error
			...
			const canSaveElevated = resource.scheme === Schemas.file; // currently only supported for local schemes (github.com/riftsh/opencursor/issues/48659)
		...
		const handle = this.notificationService.notify({
			id: `${hash(model.resource.toString())}`, // unique per model (github.com/riftsh/opencursor/issues/121539)
