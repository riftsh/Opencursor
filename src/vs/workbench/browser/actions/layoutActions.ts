...
import { AuxiliaryBarVisibleContext, PanelAlignmentContext, PanelVisibleContext, SideBarVisibleContext, FocusedViewContext, InEditorZenModeContext, IsMainEditorCenteredLayoutContext, MainEditorAreaVisibleContext, IsMainWindowFullscreenContext, PanelPositionContext, IsAuxiliaryWindowFocusedContext, TitleBarStyleContext, OpenCursorVisibleContext, OpenCursorCreatorVisibleContext } from '../../common/contextkeys.js';
...
import { TogglePearOverlayAction } from '../parts/overlay/openCursorOverlayActions.js';
...
const opencursorIcon = registerIcon('opencursor', Codicon.archive, localize('opencursorIcon', "Represents the OpenCursor overlay"));
...
MenuRegistry.appendMenuItems([{
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar left', "Move OpenCursor Side Bar Left")
		},
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar right', "Move OpenCursor Side Bar Right")
		},
			...
			containerModel.visibleViewDescriptors.forEach(viewDescriptor => {
				if (viewDescriptor.canMoveView) {
					if (!hasAddedView) {
						results.push({
							type: 'separator',
							label: localize('secondarySideBarContainer', "OpenCursor Side Bar / {0}", containerModel.title)
						});
						hasAddedView = true;
					}
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		if (!(isViewSolo && currentLocation === ViewContainerLocation.AuxiliaryBar)) {
			items.push({
				...
				label: localize('moveFocusedView.newContainerInSidePanel', "New OpenCursor Side Bar Entry")
			});
		}
		...
		items.push({
			type: 'separator',
			label: localize('secondarySideBar', "OpenCursor Side Bar")
		});
...
ToggleVisibilityActions.push(...[
	...
	CreateToggleLayoutItem(ToggleAuxiliaryBarAction.ID, AuxiliaryBarVisibleContext, localize('secondarySideBar', "OpenCursor Side Bar"), { whenA: ContextKeyExpr.equals('config.workbench.sideBar.location', 'left'), iconA: panelRightIcon, iconB: panelLeftIcon }),
	CreateToggleLayoutItem(TogglePanelAction.ID, PanelVisibleContext, localize('panel', "Panel"), panelIcon),
	CreateToggleLayoutItem(ToggleStatusbarVisibilityAction.ID, ContextKeyExpr.equals('config.workbench.statusBar.visible', true), localize('statusBar', "Status Bar"), statusBarIcon),
	CreateToggleLayoutItem(TogglePearOverlayAction.ID, OpenCursorVisibleContext, 'OpenCursor', opencursorIcon),
	CreateToggleLayoutItem(ToggleCreatorOverlayAction.ID, OpenCursorCreatorVisibleContext, 'OpenCursor Creator'),
]);
...
registerAction2(class OpenCursorSettingsAction extends Action2 {
	constructor() {
		super({
			id: 'workbench.action.opencursorSettings',
			title: localize2('opencursorSettings', "OpenCursor Settings"),
			f1: true,
	...
	run(accessor: ServicesAccessor): void {
		const commandService = accessor.get(ICommandService);
		commandService.executeCommand("opencursor.toggleOverlay")
	}
});
