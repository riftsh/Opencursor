...
export class MultiEditorTabsControl extends EditorTabsControl {
	...
	private updateTabSizing(fromEvent: boolean): void {
		...
		if (options.tabSizing === 'fixed') {
			...
			// For github.com/riftsh/opencursor/issues/40290 we want to
	...
	private registerTabsContainerListeners(tabsContainer: HTMLElement, tabsScrollbar: ScrollableElement): void {
		...
		// Prevent auto-scrolling (github.com/riftsh/opencursor/issues/16690)
		...
		// Prevent auto-pasting (github.com/riftsh/opencursor/issues/201696)
		if (isLinux) {
		...
		this._register(addDisposableListener(tabsContainer, EventType.MOUSE_WHEEL, (e: WheelEvent) => {
			...
			// Ignore event if the last one happened too recently (github.com/riftsh/opencursor/issues/96409)
	...
	private registerTabListeners(tab: HTMLElement, tabIndex: number, tabsContainer: HTMLElement, tabsScrollbar: ScrollableElement): IDisposable {
		...
		const handleClickOrTouch = async (e: MouseEvent | GestureEvent, preserveFocus: boolean): Promise<void> => {
			...
			if (isMouseEvent(e) && (e.button !== 0 /* middle/right mouse button */ || (isMacintosh && e.ctrlKey /* macOS context menu */))) {
				if (e.button === 1) {
					e.preventDefault(); // required to prevent auto-scrolling (github.com/riftsh/opencursor/issues/16690)
				}
		...
		disposables.add(addDisposableListener(tab, EventType.AUXCLICK, e => {
			if (e.button === 1 /* Middle Button*/) {
				EventHelper.stop(e, true /* for github.com/riftsh/opencursor/issues/56715 */);
		...
		}, true /* use capture to fix github.com/riftsh/opencursor/issues/19145 */));
	...
	private isSupportedDropTransfer(e: DragEvent): boolean {
		...
		if (e.dataTransfer && e.dataTransfer.types.length > 0) {
			return true; // optimistically allow external data (// see github.com/riftsh/opencursor/issues/25789)
		}
	...
	private redrawTabLabel(editor: EditorInput, tabIndex: number, tabContainer: HTMLElement, tabLabelWidget: IResourceLabel, tabLabel: IEditorInputLabel): void {
		...
		if (tabLabel.ariaLabel) {
			...
			// More details github.com/riftsh/opencursor/issues/95378
		...
		if (this.groupsView.partOptions.wrapTabs) {
			...
			const lastTabFitsWrapped = () => {
				...
				if (lastTabOverlapWithToolbarWidth > 1) {
					...
					// github.com/riftsh/opencursor/issues/116385
					return false;
				}
			...
			// flickering issue github.com/riftsh/opencursor/issues/115050)
	...
	private doLayoutTabsNonWrapping(options?: IMultiEditorTabsControlLayoutOptions): void {
		...
		if (
			...
			(!dimensionsChanged && !options?.forceRevealActiveTab) 	// dimensions did not change and we have low layout priority (github.com/riftsh/opencursor/issues/133631)
		) {
...
registerThemingParticipant((theme, collector) => {
	...
	// - if we have a contrast border (which draws an outline - github.com/riftsh/opencursor/issues/109117)
	// - on Safari (github.com/riftsh/opencursor/issues/108996)
