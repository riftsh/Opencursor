...
export class PaneCompositeBar extends Disposable {
	...
	private createMoveAction(viewContainer: ViewContainer, newLocation: ViewContainerLocation, defaultLocation: ViewContainerLocation): IAction {
		return toAction({
			...
			label: newLocation === ViewContainerLocation.Panel ? localize('panel', "Panel") : newLocation === ViewContainerLocation.Sidebar ? localize('sidebar', "Primary Side Bar") : localize('auxiliarybar', "OpenCursor Side Bar"),
			run: () => {
...
class ViewContainerActivityAction extends CompositeBarAction {
	...
	override async run(event: { preserveFocus: boolean }): Promise<void> {
		...
		const now = Date.now();
		if (now > this.lastRun /* github.com/riftsh/opencursor/issues/25830 */ && now - this.lastRun < ViewContainerActivityAction.preventDoubleClickDelay) {
			return;
		}
		this.lastRun = now;
