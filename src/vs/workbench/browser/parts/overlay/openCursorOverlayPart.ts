...
// const OPENCURSOR_FIRST_LAUNCH_KEY = "opencursor.firstLaunch";

const OPENCURSOR_CHAT_ID = "opencursor.chatView";
const PEAR_OVERLAY_TITLE = "opencursor.overlayView";

export class OpenCursorOverlayPart extends Part {
	static readonly ID = "workbench.parts.opencursor-overlay";
	...
	constructor(
		...
		super(
			OpenCursorOverlayPart.ID,
			{ hasTitle: false },
			themeService,
			storageService,
			layoutService,
		);
		this.isFirstLaunch = false; // !storageService.getBoolean(OPENCURSOR_FIRST_LAUNCH_KEY, 0);
	...
	private async initialize() {
		...
		if (this.isFirstLaunch) {
			...
			// setting the theme to OpenCursor Dark on first launch here to avoid flicker
			this._instantiationService.invokeFunction((accessor) => {
				const configurationService = accessor.get(IConfigurationService);
				configurationService.updateValue(
					"workbench.colorTheme",
					"Default OpenCursor Dark",
				);
			});
		} else {
			this.state = "closed";
		...
		// OpenCursor: Force hide any stuck loading messages
		setTimeout(() => {
		...
		const extensionDescription: WebviewExtensionDescription = {
			id: new ExtensionIdentifier(OPENCURSOR_CHAT_ID),
			location: URI.parse(""),
		};
		...
		// OpenCursor: Extension resolved, force hide overlay
		this.hideOverlayLoadingMessage();
		this.unlock();
	...
	protected override createContentArea(element: HTMLElement): HTMLElement {
		// create the full screen overlay. this serves as a click target for closing opencursor
		this.element = element;
		this.fullScreenOverlay = element; // use the openCursorOverlayPart root element as the fullScreenOverlay
		...
		this.popupAreaOverlay = $("div.opencursor-popup-area-overlay");
		...
		if (this.isFirstLaunch) {
			...
			this.loadingOverlay = $("div.opencursor-loading-overlay");
	...
	private async open() {
		...
		this._commandService.executeCommand("opencursor.notifyOverlayOpened");
	...
	private close() {
		...
		container.style.animation = "opencursorFadeOut 0.2s ease-out";
	...
	show(): void {
		if (this.state === "loading") {
			console.warn("Can't open OpenCursor while loading");
			return;
		}
	...
	hide(): void {
		if (this.state === "loading") {
			console.warn("Can't close OpenCursor while loading");
			return;
		}
		this.close();
	}
	...
	toggle(): void {
		if (this.state === "loading") {
			console.warn("Can't toggle OpenCursor while loading");
			return;
		}
	...
	toJSON(): object {
		return {
			type: Parts.OPENCURSOR_OVERLAY_PART,
		};
	}
}
