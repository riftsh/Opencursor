	...
	implements ICreatorOverlayService
{
	...
	private registerCommands(): void {
		...
		CommandsRegistry.registerCommand(
			"opencursor.isCreatorOverlayVisible",
			(accessor) => {
		...
		CommandsRegistry.registerCommand("opencursor.toggleCreator", (accessor) => {
			const overlayService = accessor.get(ICreatorOverlayService);
			overlayService.toggle();
		});

		CommandsRegistry.registerCommand(
			"opencursor.lockCreatorOverlay",
			(accessor) => {
				const overlayService = accessor.get(ICreatorOverlayService);
				overlayService.lock();
			},
		...
		CommandsRegistry.registerCommand(
			"opencursor.unlockCreatorOverlay",
			(accessor) => {
				const overlayService = accessor.get(ICreatorOverlayService);
		...
		CommandsRegistry.registerCommand(
			"opencursor.isCreatorOverlayLocked",
			(accessor) => {
				const overlayService = accessor.get(ICreatorOverlayService);
