...
import { OpenCursorChatExtensionId, OpenCursorSearchExtensionId, OpenCursorMemoryExtensionId, OpenCursorRooExtensionId, OpenCursorView, OPENCURSOR_VIEWS} from '../../../services/views/opencursor/opencursorViewsShared.js';
...
export class SwitchToOpenCursorIntegrationAction extends Action2 {
  static readonly ID = 'workbench.action.switchToOpenCursorIntegrationIconBar';

  constructor() {
    super({
      id: SwitchToOpenCursorIntegrationAction.ID,
    title: localize2('switchToOpenCursorView', "Switch to OpenCursor Integration"),
    category: Categories.View,
    f1: true
    });
  }

  override async run(accessor: ServicesAccessor, args?: { view: OpenCursorView }): Promise<void> {
    ...
    await viewsService.openView(OPENCURSOR_VIEWS[view], true);
  }
}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('closeSecondarySideBar', 'Hide OpenCursor Side Bar'),
			category: Categories.View,
			icon: closeIcon,
			menu: [{
				id: MenuId.CommandPalette,
...
// Move OpenCursor extension to OpenCursor Side Bar (Auxiliary Bar) (we want OpenCursor Side Bar to be default loaction for extension)
class MovePearExtensionToAuxBarAction extends MoveViewsBetweenPanelsAction {
    ...
    override run(accessor: ServicesAccessor): void {
        ...
        const chatViewContainer = viewDescriptorService.getViewContainerById(OpenCursorChatExtensionId);
        const searchViewContainer = viewDescriptorService.getViewContainerById(OpenCursorSearchExtensionId);
        const memoryViewContainer = viewDescriptorService.getViewContainerById(OpenCursorMemoryExtensionId);
        const agentViewContainer = viewDescriptorService.getViewContainerById(OpenCursorRooExtensionId);
...
// --- Move Panel Views To OpenCursor Side Bar
...
class MovePanelToSidePanelAction extends MoveViewsBetweenPanelsAction {
	...
	constructor() {
		super(ViewContainerLocation.Panel, ViewContainerLocation.AuxiliaryBar, {
			...
			title: localize2('movePanelToSecondarySideBar', "Move Panel Views To OpenCursor Side Bar"),
			category: Categories.View,
			f1: false
		});
	}
...
// --- Move OpenCursor Side Bar Views To Panel
...
class MoveSidePanelToPanelAction extends MoveViewsBetweenPanelsAction {
	...
	constructor() {
		super(ViewContainerLocation.AuxiliaryBar, ViewContainerLocation.Panel, {
			...
			title: localize2('moveSidePanelToPanel', "Move OpenCursor Side Bar Views To Panel"),
			category: Categories.View,
			f1: false
		});
	}
