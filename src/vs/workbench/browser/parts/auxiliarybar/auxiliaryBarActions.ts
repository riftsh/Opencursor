...
export class ToggleAuxiliaryBarAction extends Action2 {
	...
	static readonly LABEL = localize2('toggleAuxiliaryBar', "Toggle OpenCursor Side Bar Visibility");
	...
	constructor() {
		super({
			...
			toggled: {
				...
				title: localize('secondary sidebar', "OpenCursor Side Bar"),
...
registerAction2(class FocusAuxiliaryBarAction extends Action2 {
	...
	static readonly LABEL = localize2('focusAuxiliaryBar', "Focus into OpenCursor Side Bar");
...
MenuRegistry.appendMenuItems([
	{
		...
		item: {
			...
			command: {
				id: ToggleAuxiliaryBarAction.ID,
				title: localize('toggleSecondarySideBar', "Toggle OpenCursor Side Bar"),
		...
		item: {
			...
			command: {
				id: ToggleAuxiliaryBarAction.ID,
				title: localize2('hideAuxiliaryBar', 'Hide OpenCursor Side Bar'),
			},
...
registerAction2(class extends SwitchCompositeViewAction {
	constructor() {
		super({
			...
			title: localize2('previousAuxiliaryBarView', 'Previous OpenCursor Side Bar View'),
			category: Categories.View,
			f1: true
...
registerAction2(class extends SwitchCompositeViewAction {
	constructor() {
		super({
			...
			title: localize2('nextAuxiliaryBarView', 'Next OpenCursor Side Bar View'),
			category: Categories.View,
			f1: true
		}, ViewContainerLocation.AuxiliaryBar, 1);
	}
...
class FocusOpenCursorExtensionAction extends Action2 {
	static readonly ID = 'workbench.action.focusOpenCursorExtension';
	static readonly LABEL = localize2(
		"focusOpenCursorExtension",
		"Focus into OpenCursor Extension",
	);

	constructor() {
		super({
			id: FocusOpenCursorExtensionAction.ID,
			title: FocusOpenCursorExtensionAction.LABEL,
			category: Categories.View,
			f1: true,
			// keybinding: do not add keybinding CTRL/CMD L here, it comes from opencursor extension
		});
	}

	override async run(accessor: ServicesAccessor): Promise<void> {
		// focus opencursor extension
		const commandService = accessor.get(ICommandService);
		commandService.executeCommand('opencursor.focusContinueInput');
	}
}

registerAction2(FocusOpenCursorExtensionAction);

// Following is a only OpenCursor related action, need to refactor these type of actions to separate file

class OpenOpenCursorDocsAction extends Action2 {
	static readonly ID = 'workbench.action.openOpenCursorDocs';
	static readonly LABEL = localize2(
		"openOpenCursorDocs",
		"Open OpenCursor Documentation",
	);

	constructor() {
		super({
			id: OpenOpenCursorDocsAction.ID,
			title: OpenOpenCursorDocsAction.LABEL,
...
registerAction2(OpenOpenCursorDocsAction);
