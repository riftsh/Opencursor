...
app_dir=$(pwd)
target_path="$app_dir/extensions/opencursor-submodule/extensions/vscode"
link_path="$app_dir/extensions/opencursor-ref"

# Run the base functionality
echo -e "\nInitializing sub-modules..."

# Check if the submodule directory already exists
if [ -d "$app_dir/extensions/opencursor-submodule" ]; then
    echo "Removing existing opencursor-submodule directory"
    execute "rm -rf $app_dir/extensions/opencursor-submodule" "Failed to remove existing opencursor-submodule directory"
fi

# Check if the OpenCursor Roo Code directory already exists
if [ -d "$app_dir/extensions/OpenCursor-Roo-Code" ]; then
    echo "Removing existing OpenCursor-Roo-Code directory"
    execute "rm -rf $app_dir/extensions/OpenCursor-Roo-Code" "Failed to remove existing OpenCursor-Roo-Code directory"
fi
...
execute "cd ./extensions/opencursor-submodule" "Failed to change directory to extensions/opencursor-submodule"
...
# ROO CODE
execute "cd ../OpenCursor-Roo-Code" "Failed to change directory to extensions/OpenCursor-Roo-Code"
# Discard any potential changes or merge conflicts in the working directory or staging area,
# ensuring local branch matches remote branch exactly before checking out main
execute "git reset --hard" "Failed to reset --hard"
execute "git checkout main" "Failed to checkout main branch"
...
# Make sure the submodule has the latest updates
execute "git pull origin main" "Failed to pull latest changes from origin/main"
execute "npm run install:all" "Failed to install dependencies for the OpenCursor-Roo-Code"
execute "npm run build" "Failed to build the OpenCursor-Roo-Code"
