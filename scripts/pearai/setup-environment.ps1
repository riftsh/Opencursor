...
function Initialize-BaseFunctionality {
    ...
    Set-Location .\extensions\opencursor-submodule
...
function Create-SymLink {
    Write-Host "`nCreating symbolic link 'extensions\opencursor-ref' -> 'extensions\opencursor-submodule\extensions\vscode'" -ForegroundColor White
...
$modulePath = Join-Path -Path $currentDir -ChildPath 'extensions\opencursor-submodule'
$targetPath = Join-Path -Path $modulePath -ChildPath 'extensions\vscode'
$linkPath = Join-Path -Path $currentDir -ChildPath 'extensions\opencursor-ref'
