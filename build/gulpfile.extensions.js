...
const compilations = [
	...
	if (process.env.FAST_BUILD === 'true') {
		// Only include core extensions or those likely to be modified in OpenCursor
		return path.includes('opencursor') || path.includes('vscode-web') || path.includes('markdown');
	}
	return true;
});
...
const tasks = compilations.map(function (tsconfigFile) {
	...
	const compileBuildTask = task.define(`compile-build-extension-${name}`, () => {
